(ns argo-ocean-floats.main
  "Kotodama WASM entrypoint for the Argo_ocean_floats clean-room actor (L4) — Clojure port.
  ...
  facts (`argo_ocean_floats.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "argo_ocean_floats")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"argo_ocean_floats.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "argo_ocean_floats-compat" :tier tier :entities entities} 200])
...
(defn start! [] :argo_ocean_floats-compat/ready)
