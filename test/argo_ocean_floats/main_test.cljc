(ns argo-ocean-floats.main-test
  "Contract + behavioral test for the argo_ocean_floats-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "argo_ocean_floats-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'argo-ocean-floats.main-test)]
